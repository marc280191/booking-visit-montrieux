  ...
  for (let i = 0; i < horizon; i += 1) {
    ...
    for (const [startTime, endTime] of slots) {
      await db.bookingSlot.upsert({
        where: {
          shopId_date_startTime_endTime: { shopId: shop.id, date, startTime, endTime },
        },
        update: {},
        create: {
          shopId: shop.id,
...
export async function listSlotsByDate(shopDomain: string, date: Date) {
  ...
  return db.bookingSlot.findMany({
    where: { shopId: shop.id, date: startOfDay(date) },
...
export async function recalcSlotCapacity(slotId: string) {
  ...
  return db.bookingSlot.update({
    ...
    data: {
      ...
      status: slot.isBlocked ? SlotStatus.BLOCKED : reserved >= slot.capacityMax ? SlotStatus.FULL : SlotStatus.AVAILABLE,
    },
  });
}
...
export async function blockSlot(slotId: string, reason?: string) {
  return db.bookingSlot.update({
    where: { id: slotId },
    data: { isBlocked: true, blockReason: reason ?? null, status: 'BLOCKED' },
  });
}

export async function unblockSlot(slotId: string) {
  const slot = await db.bookingSlot.update({
    where: { id: slotId },
    data: { isBlocked: false, blockReason: null },
  });
  await recalcSlotCapacity(slotId);
  return slot;
}
